...
export default function ServiceProviderList({ categories }: Props) {
  ...
  const sendContactRequest = (provider: Provider, method: 'email' | 'phone') => {
    if (method === 'email' && provider.email) {
      window.location.href = `mailto:${provider.email}?subject=Anfrage für Gartenarbeit (Refugium Heideland)`;
              ...
              <div className="divide-y divide-gray-100">
                {!hasProviders ? (
                  <div className="p-6 text-center">
                    ...
                    <a
                      href={`mailto:garten@infinityspace42.de?subject=Neuer ${category.name} für Refugium Heideland`}
